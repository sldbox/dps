...
(() => {
  ...
  const MODES = ['is-pc-landscape', 'is-pc-portrait', 'is-tablet', 'is-mobile', 'is-portrait-view', 'is-mobile-device', 'is-tablet-device', 'is-tablet-portrait', 'is-tablet-landscape', 'is-narrow-mobile', 'is-tabbed'];
  const MOBILE_PAGES = [
  ...
  function getViewportProfile() {
    ...
    const portrait = h > w;
    const mobile = shortSide <= 600;
    const tablet = !mobile && shortSide <= 1024 && longSide <= 1366;
    const tabletPortrait = tablet && portrait;
    const tabletLandscape = tablet && !portrait;
    const mode = mobile ? 'is-mobile' : (tabletPortrait ? 'is-tablet' : (portrait ? 'is-pc-portrait' : 'is-pc-landscape'));

    return { w, h, shortSide, longSide, portrait, mobile, tablet, tabletPortrait, tabletLandscape, mode };
  }

  function getMode() {
    return getViewportProfile().mode;
  }
  ...
  function applyMode() {
    ...
    const direction = profile.portrait ? 'is-portrait-view' : '';
    const deviceClass = profile.mobile ? 'is-mobile-device' : (profile.tablet ? 'is-tablet-device' : '');
    const tabletOrientationClass = profile.tabletPortrait ? 'is-tablet-portrait' : (profile.tabletLandscape ? 'is-tablet-landscape' : '');
    const widthClass = profile.mobile && profile.shortSide <= 430 ? 'is-narrow-mobile' : '';
    const tabbed = profile.mobile || profile.tabletPortrait;
    ...
    if (tabletOrientationClass) {
      document.body.classList.add(tabletOrientationClass);
      document.documentElement.classList.add(tabletOrientationClass);
    }
    if (widthClass) {
    ...
    state.layoutWidth = profile.w;
    state.layoutPortrait = profile.portrait;
    syncMobileLayout();
    updateMobileOffsets();
  }
