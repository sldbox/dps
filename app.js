...
const ROUND_DISABLED_LABEL='일반 난이도 활성화 필요';
const TOWER_FLOOR_DISABLED_LABEL='도전의 탑 활성화 필요';
...
function targetRoundStoredValue(){
  ...
  const source=(el.disabled && el.value===ROUND_DISABLED_LABEL) ? el.dataset.roundValue : el.value;
  return normalizedRoundString(source || ROUND_INPUT_MIN);
}
function challengeTowerFloorStoredValue(){
  const el=$('challengeTowerFloor');
  if(!el) return String(TOWER_FLOOR_INPUT_MIN);
  const source=(el.disabled && el.value===TOWER_FLOOR_DISABLED_LABEL) ? el.dataset.challengeTowerFloorValue : (el.value || el.dataset.challengeTowerFloorValue);
  return normalizedTowerFloorString(source || TOWER_FLOOR_INPUT_MIN);
}
...
const PENANCE_GENERAL_DISABLED_LABEL='일반 난이도 활성화 필요';
const PENANCE_DEEP_DISABLED_LABEL='고행 단계 비활성화';
...
function isPenanceControlDisabled(diffName=vs('diff')){
  const name=difficultyName(diffName);
  return name===TOWER_DIFFICULTY_NAME || name==='Deep Abyss';
}
function penanceDisabledLabel(diffName=vs('diff')){
  return difficultyName(diffName)===TOWER_DIFFICULTY_NAME ? PENANCE_GENERAL_DISABLED_LABEL : PENANCE_DEEP_DISABLED_LABEL;
}
...
function penanceStoredValue(){
  const el=$('penance');
  if(!el) return '0';
  const raw=el.disabled ? el.dataset.penanceValue : el.value;
  return normalizePenanceValue(raw, SOLO_PENANCE_MAX);
}
function effectivePenanceValue(){
  return isPenanceControlDisabled() ? 0 : Number(normalizePenanceValue(penanceStoredValue(), currentPenanceMax()));
}
function penanceOptionLabel(value){return value>0 ? `${value} 고행` : '없음';}
function syncPenanceOptions(){
  const el=$('penance');
  if(!el) return;
  const disabled=isPenanceControlDisabled();
  const stored=normalizePenanceValue(el.dataset.penanceValue || (el.value && !el.disabled ? el.value : '0'), SOLO_PENANCE_MAX);
  el.dataset.penanceValue=stored;
  if(disabled){
    const label=penanceDisabledLabel();
    const signature=`disabled:${label}`;
    if(el.dataset.penanceMax!==signature){
      setSelectOptions(el, [{value:'',label,selected:true}]);
  ...
  const signature=String(max);
  if(el.dataset.penanceMax!==signature){
    setSelectOptions(el, Array.from({length:max+1}, (_,value)=>({value,label:penanceOptionLabel(value)})));
    el.dataset.penanceMax=signature;
  }
  const next=normalizePenanceValue(stored, max);
  el.disabled=false;
  el.value=next;
  el.dataset.penanceValue=next;
}
...
function normalizeRoundInput(id='round'){
  ...
  if(!el) return false;
  if(id==='round'){
    if(el.disabled && el.value===ROUND_DISABLED_LABEL) return false;
  ...
  if(id==='challengeTowerFloor'){
    if(el.disabled && el.value===TOWER_FLOOR_DISABLED_LABEL) return false;
...
function syncDifficultyTargetControls(){
  ...
  if(roundEl){
    if(towerActive){
      const stored=normalizedRoundString(roundEl.dataset.roundValue || (roundEl.value!==ROUND_DISABLED_LABEL ? roundEl.value : ROUND_INPUT_MIN));
      roundEl.dataset.roundValue=stored;
      roundEl.value=ROUND_DISABLED_LABEL;
  ...
  if(towerEl){
    if(towerActive){
      const stored=normalizedTowerFloorString(towerEl.dataset.challengeTowerFloorValue || (towerEl.value!==TOWER_FLOOR_DISABLED_LABEL ? towerEl.value : TOWER_FLOOR_INPUT_MIN));
      towerEl.disabled=false;
      towerEl.value=stored;
      towerEl.dataset.challengeTowerFloorValue=stored;
    }else{
      const stored=normalizedTowerFloorString(towerEl.dataset.challengeTowerFloorValue || (towerEl.value!==TOWER_FLOOR_DISABLED_LABEL ? towerEl.value : TOWER_FLOOR_INPUT_MIN));
      towerEl.dataset.challengeTowerFloorValue=stored;
      towerEl.value=TOWER_FLOOR_DISABLED_LABEL;
      towerEl.disabled=true;
    }
  }
}
function resetDifficultyDependentFields(){
  ...
  let changed=false;
  if(pen){
    const current=penanceStoredValue();
    if(current!=='0'){ changed=true; }
    pen.dataset.penanceValue='0';
    if(!pen.disabled) pen.value='0';
  }
  syncPenanceOptions();
...
function computeDpsPreview(diffName, penanceLevel, round, options={}){
  const ids=['diff','penance','round','challengeTowerFloor','soloMode','coopMode','coopPlayers','team','pbless'];
  const saved=ids.map(id=>{
    const el=$(id);
    return [el, el ? el.value : null, el ? el.disabled : false, el ? el.dataset.roundValue : undefined, el ? el.dataset.challengeTowerFloorValue : undefined];
  });
  try{
    const diffEl=$('diff');
    const penEl=$('penance');
    ...
    if(towerFloorEl && diffName===TOWER_DIFFICULTY_NAME){ towerFloorEl.value=String(round); towerFloorEl.dataset.challengeTowerFloorValue=normalizedTowerFloorString(round); }
    ...
    saved.forEach(([el,val,disabled,roundValue,towerFloorValue])=>{
      if(!el) return;
      el.value=val;
      el.disabled=disabled;
      if(roundValue===undefined) delete el.dataset.roundValue; else el.dataset.roundValue=roundValue;
      if(towerFloorValue===undefined) delete el.dataset.challengeTowerFloorValue; else el.dataset.challengeTowerFloorValue=towerFloorValue;
    });
  }
}
...
function buildCompareHeaderControls(){
  return `<div class="excel-compare-controls excel-compare-header-controls">
    ...
    <select id="excelCompareSheet" aria-label="비교 프리셋 목록" disabled><option>비교 프리셋 목록</option></select>
...
function writeElementValue(el, value){
  ...
  if(el.id==='penance'){
    ...
    el.dataset.penanceValue=stored;
    value=isPenanceControlDisabled() ? '' : normalizePenanceValue(stored, currentPenanceMax());
  }
  if(el.id==='round'){
    const stored=normalizedRoundString(value);
    el.dataset.roundValue=stored;
    value=isTowerDifficulty() ? ROUND_DISABLED_LABEL : stored;
  }
  if(el.id==='challengeTowerFloor'){
    const stored=normalizedTowerFloorString(value);
    el.dataset.challengeTowerFloorValue=stored;
    value=isTowerDifficulty() ? stored : TOWER_FLOOR_DISABLED_LABEL;
  }
...
function sanitizeSavedValues(values, context={}){
  ...
  if(Object.prototype.hasOwnProperty.call(out,'penance')){
    if(isPenanceControlDisabled(out.diff)){
      out.penance='0';
    }else{
...
function compareTraitPreset(){
  ...
  if(preset){
    hydrateCompareControls();
    compareSelectedTraitPreset();
    showToast(`프리셋 분석에 연결: ${preset.name}`,'ok');
  }
  return true;
}
