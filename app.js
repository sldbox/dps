...
/* Configuration and calculator state */
var DPS_CONFIG={
  storage:{
    version:(window.DPS_BUILD_VERSION || window.DPS_VERSION || 'dev'),
    scope:'browser_local',
...
/* Shared DOM, input, and notification helpers */
...
function buildExcelInputRows(cells,specCells){
  const titleBonusMap={'패왕':'12','패왕+':'13','제왕':'14','제왕+':'15','신황':'16','신황+':'17'};
  const runeTypeMap={'AP':'ap','UA':'ua','TD':'td','TD&UA':'harmony','TD＆UA':'harmony'};
  const rows=[
    ...
    ['기본정보','승단 총데미지',titleBonusMap[excelText(specCells.S17)]??specCells.S17,'titleTdBonus'],
    ...
    ['룬정보','특수룬 종류',runeTypeMap[excelText(cells.I6)]??cells.I6,'runeChoiceType'],
  ...
  return rows.map(([kind,name,excel,id])=>{
    const excelValue=String(excel??'').replace(/,/g,'').trim();
    const webValue=String(webControlDisplay(id)).replace(/,/g,'').trim();
    const same=excelValue.toLowerCase()===webValue.toLowerCase();
    return {kind,name,excel:escapeCompareHtml(excelValue),web:escapeCompareHtml(webValue),
      difference:same?'일치':`${excelValue} → ${webValue}`,status:same?'same':'diff'};
  });
}
...
function applyExcelControl(id, value, options={}){
  const el=document.getElementById(id);
  if(!el || value===undefined || value===null || value==='') return false;
  let next=value;
  if(el.tagName==='SELECT'){
    next=options.valueMap?.[excelText(value)] ?? resolveExcelSelectValue(id,value);
    if(next===null||next===undefined) return false;
  }else if(el.type==='checkbox'){
    next=excelFlag(value);
  }else if(options.number){
    next=excelNumber(value);
    if(next===null) return false;
    if(options.integer) next=Math.round(next);
  }
  writeElementValue(el,next);
  return true;
}
...
function syncWebInputsFromExcel(cells){
  let changed=false;
  const mappings=[
    ['diff',['B4','N41']],
    ['penance',['B6','N42','AD8']],
  ...
  mappings.forEach(([id,refs])=>{
    const value=firstExcelValue(cells,refs);
    if(value!==null && applyExcelControl(id,value)) changed=true;
  });
  if(changed){
    recalc();
    saveState({silent:true});
  }
  const armor=excelNumber(cells.AF8);
  if(armor!==null){
    setValue('enemyArmor',armor);
    setText('enemyArmorQuick',big(armor));
  }
}
...
function buildExcelState(cells, specCells){
  ...
  [
    ...
    ['addAD',specCells.R36],['addAS',specCells.R37],['addCD',specCells.R38],
    ['addCRI',specCells.R39],['addAP',specCells.R40],['addTD',specCells.R41],['addUA',specCells.R42]
  ...
  const titleBonusMap={'패왕':'12','패왕+':'13','제왕':'14','제왕+':'15','신황':'16','신황+':'17'};
  applied+=assign('titleTdBonus',specCells.S17,{valueMap:titleBonusMap});
  const runeTypeMap={'AP':'ap','UA':'ua','TD':'td','TD&UA':'harmony','TD＆UA':'harmony'};
  applied+=assign('runeChoiceType',cells.I6,{valueMap:runeTypeMap});
  applied+=assign('runeChoiceValue',cells.J6,{number:true});
  const inv={...state.inv};
...
function applySelectedExcelSheet(){
  ...
  try{
    ...
    if(!Number.isFinite(Number(cells.M19))||!Number.isFinite(Number(cells.L4))){
      throw new Error(`"${select.value}" 시트는 현재 계산기에 적용할 수 있는 셀 구조가 아닙니다.`);
    }
...
/* Trait board rendering, investment, and optimization */
...
/* Save, restore, backup, and local storage */
...
/* Event routing and application bootstrap */
