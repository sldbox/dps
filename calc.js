...
function dpsBaseUnitRegularList(){
  return dpsBaseUnitList().filter(unit=>!unit.isArtifact);
}
...
function normalizeDpsBaseUnitsValue(value){
  ...
  source.forEach(item=>{
    ...
    if(id===allId){ ids.length=0; ids.push(allId); return; }
    if(!validIds.has(id) || ids.includes(allId)) return;
    if(id===artifactId){ ids.length=0; ids.push(id); return; }
    if(ids.includes(artifactId)) ids.length=0;
    ids.push(id);
  });
  return ids.includes(allId) ? allId : ids.join(DPS_BASE_UNIT_STORAGE_SEPARATOR);
}
...
function selectedDpsBaseUnits(value=dpsBaseUnitStorageValue()){
  ...
  if(ids.includes(dpsBaseUnitAllId())) return dpsBaseUnitRegularList();
...
function selectedArtifactDpsBaseWeaponAttack(){
  const artifactId=dpsBaseUnitArtifactId();
  if(!dpsBaseUnitSelectionIds(dpsBaseUnitStorageValue()).includes(artifactId)) return 0;
