/* ===== animation.js | 전투·특성 프리셋 상황 애니메이션 ===== */
(function(){
  'use strict';
  ...
  /* ===== 특성 프리셋 보드: 생활형 고양이 상태 머신 ===== */
  const PRESET_CAT_ACTIONS=Object.freeze({
    select:Object.freeze({message:'이 프리셋을 선택했어!',caption:'목록 확인',pose:'listening',x:'51%',y:'17px',duration:2300,priority:20}),
    load:Object.freeze({message:'프리셋을 불러왔어!',caption:'불러오기 완료',pose:'package',x:'79%',y:'17px',facing:-1,duration:3000,priority:45}),
    save:Object.freeze({message:'새 프리셋을 저장했어!',caption:'저장 완료',pose:'happy',x:'47%',y:'18px',duration:3200,priority:48}),
    rename:Object.freeze({message:'프리셋 이름을 바꿨어!',caption:'이름 변경 완료',pose:'nameplate',x:'34%',y:'18px',duration:3000,priority:42}),
    update:Object.freeze({message:'최신 상태로 업데이트했어!',caption:'업데이트 완료',pose:'check',x:'48%',y:'18px',duration:3300,priority:55}),
    import:Object.freeze({message:'프리셋을 가져왔어!',caption:'가져오기 완료',pose:'package',x:'80%',y:'17px',facing:-1,duration:3300,priority:52}),
    export:Object.freeze({message:'안전하게 내보냈어!',caption:'내보내기 완료',pose:'dispatch',x:'82%',y:'17px',facing:-1,duration:3300,priority:52}),
    'list-change':Object.freeze({message:'프리셋 목록을 정리했어!',caption:'목록 변경',pose:'listening',x:'51%',y:'17px',duration:2700,priority:28}),
    'need-update':Object.freeze({message:'변경사항이 있어. 업데이트가 필요해!',caption:'업데이트 필요',pose:'alert',x:'50%',y:'18px',duration:4300,priority:90,attention:true}),
    'need-export':Object.freeze({message:'변경한 프리셋을 내보내야 해!',caption:'내보내기 필요',pose:'urgent-door',x:'81%',y:'17px',facing:-1,duration:4100,priority:82,attention:true}),
    'need-import':Object.freeze({message:'저장된 프리셋이 없어. 가져오기 해줘!',caption:'가져오기 필요',pose:'waiting-door',x:'80%',y:'17px',facing:-1,duration:4300,priority:76,attention:true}),
    error:Object.freeze({message:'처리하지 못했어. 내용을 확인해줘!',caption:'확인 필요',pose:'error',x:'50%',y:'18px',duration:4300,priority:100})
  });
  const PRESET_CAT_ATTENTION_ACTIONS=Object.freeze([
  ...
  const PRESET_CAT_IDLE_STATES=Object.freeze([
    Object.freeze({caption:'방 안을 산책 중',pose:'walking',x:'44%',y:'17px',duration:5600}),
    Object.freeze({caption:'캣타워에서 쉬는 중',pose:'tower',x:'15%',y:'69px',duration:6500}),
    Object.freeze({caption:'창밖을 구경하는 중',pose:'window',x:'71%',y:'33px',duration:6100}),
    Object.freeze({caption:'장난감과 노는 중',pose:'playing',x:'58%',y:'17px',duration:5200}),
    Object.freeze({caption:'쿠션에서 쉬는 중',pose:'resting',x:'28%',y:'17px',duration:7200})
  ]);
  const presetCatState={
    board:null,cat:null,bubble:null,message:null,caption:null,queue:[],current:null,idleIndex:-1,
    timer:0,reminderTimer:0,attention:{needsUpdate:false,needsExport:false,needsImport:false},paused:false
  };
  function ensurePresetCatBoard(){
    const board=document.getElementById('presetCatBoard');
    if(!board) return false;
    if(presetCatState.board!==board){
      presetCatState.board=board;
      presetCatState.cat=board.querySelector('[data-preset-cat]');
      presetCatState.bubble=board.querySelector('#presetCatBubble');
      presetCatState.message=board.querySelector('[data-preset-cat-message]');
      presetCatState.caption=board.querySelector('[data-preset-cat-caption]');
    }
    return Boolean(presetCatState.cat&&presetCatState.bubble&&presetCatState.message);
  }
  function presetCatClearTimer(name){
    if(presetCatState[name]) window.clearTimeout(presetCatState[name]);
    presetCatState[name]=0;
  }
  function presetCatApplyScene(scene,options={}){
    if(!ensurePresetCatBoard()) return;
    const board=presetCatState.board;
    const cat=presetCatState.cat;
    board.dataset.presetCatMode=options.notification?'notification':'idle';
    board.dataset.presetCatPose=scene.pose||'resting';
    board.classList.toggle('is-attention',Boolean(scene.attention));
    board.classList.toggle('is-error',options.action==='error');
    const catX=scene.x||'50%';
    cat.style.setProperty('--preset-cat-x',catX);
    cat.style.setProperty('--preset-cat-y',scene.y||'17px');
    cat.style.setProperty('--preset-cat-facing',String(scene.facing||1));
    const xPercent=Number.parseFloat(catX);
    board.dataset.presetCatBubbleEdge=Number.isFinite(xPercent)&&xPercent>=72?'right':Number.isFinite(xPercent)&&xPercent<=28?'left':'center';
    if(presetCatState.caption) presetCatState.caption.textContent=scene.caption||'쉬는 중';
    if(options.notification){
      presetCatState.message.textContent=String(options.message||scene.message||'').trim();
      presetCatState.bubble.hidden=false;
    }else{
      presetCatState.bubble.hidden=true;
      presetCatState.message.textContent='';
    }
  }
  function presetCatRunIdle(){
    if(presetCatState.paused||!ensurePresetCatBoard()) return;
    presetCatState.current=null;
    presetCatState.idleIndex=(presetCatState.idleIndex+1)%PRESET_CAT_IDLE_STATES.length;
    const scene=PRESET_CAT_IDLE_STATES[presetCatState.idleIndex];
    presetCatApplyScene(scene);
    presetCatClearTimer('timer');
    presetCatState.timer=window.setTimeout(()=>{presetCatState.timer=0;presetCatAdvance();},scene.duration);
  }
  function presetCatRunNotification(entry){
    presetCatState.current=entry;
    presetCatApplyScene(entry.scene,{notification:true,action:entry.action,message:entry.message});
    presetCatClearTimer('timer');
    presetCatState.timer=window.setTimeout(()=>{
      presetCatState.timer=0;
      presetCatState.current=null;
      presetCatAdvance();
    },entry.scene.duration);
  }
  function presetCatAdvance(){
    if(presetCatState.paused) return;
    presetCatState.queue.sort((a,b)=>b.scene.priority-a.scene.priority||a.createdAt-b.createdAt);
    const next=presetCatState.queue.shift();
    if(next) presetCatRunNotification(next);
    else presetCatRunIdle();
  }
  function presetCatQueue(action,options={}){
    const scene=PRESET_CAT_ACTIONS[action];
    if(!scene||!ensurePresetCatBoard()) return false;
    const message=String(options.message||scene.message||'').trim();
    const entry={action,scene,message,createdAt:Date.now()};
    presetCatState.queue=presetCatState.queue.filter(item=>item.action!==action);
    const current=presetCatState.current;
    if(!current||scene.priority>current.scene.priority||options.replace===true){
      presetCatClearTimer('timer');
      presetCatRunNotification(entry);
    }else{
      presetCatState.queue.push(entry);
    }
    return true;
  }
  function presetCatAttentionActions(){
    return PRESET_CAT_ATTENTION_ACTIONS
      .filter(item=>presetCatState.attention[item.key])
      .map(item=>item.action);
  }
  function schedulePresetCatAttentionReminder(){
    presetCatClearTimer('reminderTimer');
    if(!presetCatAttentionActions().length) return;
    presetCatState.reminderTimer=window.setTimeout(()=>{
      presetCatState.reminderTimer=0;
      presetCatAttentionActions().forEach((action,index)=>presetCatQueue(action,{replace:index===0}));
      schedulePresetCatAttentionReminder();
    },22000);
  }
  function setPresetAttention(partial={}){
    const previous={...presetCatState.attention};
    PRESET_CAT_ATTENTION_ACTIONS.forEach(({key})=>{
      if(Object.prototype.hasOwnProperty.call(partial,key)) presetCatState.attention[key]=Boolean(partial[key]);
    });
    PRESET_CAT_ATTENTION_ACTIONS.forEach(({key,action})=>{
      if(presetCatState.attention[key]) return;
      presetCatState.queue=presetCatState.queue.filter(item=>item.action!==action);
      if(presetCatState.current?.action===action){
        presetCatClearTimer('timer');
        presetCatState.current=null;
      }
    });
    const newlyActive=PRESET_CAT_ATTENTION_ACTIONS
      .filter(({key})=>!previous[key]&&presetCatState.attention[key])
      .map(item=>item.action);
    newlyActive.forEach((action,index)=>presetCatQueue(action,{replace:index===0}));
    if(!presetCatState.current) presetCatAdvance();
    schedulePresetCatAttentionReminder();
  }
  function notifyPreset(action,options={}){
    const safeAction=PRESET_CAT_ACTIONS[action]?action:'error';
    const message=safeAction==='error'&&options.message
      ?String(options.message).replace(/\s+/g,' ').slice(0,72)
      :options.message;
    return presetCatQueue(safeAction,{...options,message});
  }
  function setPresetCatPaused(value){
    presetCatState.paused=Boolean(value);
    if(presetCatState.board) presetCatState.board.classList.toggle('is-paused',presetCatState.paused);
    if(!presetCatState.paused&&!presetCatState.timer) presetCatAdvance();
  }
  function initPresetCat(){
    if(!ensurePresetCatBoard()) return;
    setPresetCatPaused(document.hidden);
    if(!presetCatState.current&&!presetCatState.timer) presetCatRunIdle();
  }
  function refreshPresetCat(){
    if(!ensurePresetCatBoard()) return;
  }
  function destroyPresetCat(){
    presetCatClearTimer('timer');
    presetCatClearTimer('reminderTimer');
    presetCatState.queue=[];
    presetCatState.current=null;
    presetCatState.idleIndex=-1;
    presetCatState.attention={needsUpdate:false,needsExport:false,needsImport:false};
    presetCatState.paused=false;
    presetCatState.board?.classList.remove('is-attention','is-error','is-paused');
    presetCatState.board=null;presetCatState.cat=null;presetCatState.bubble=null;presetCatState.message=null;presetCatState.caption=null;
  }
  ...
  function onVisibilityChange(){paused=document.hidden;setPresetCatPaused(paused);if(!paused) queueRefresh();}
  ...
  window.Animation=Object.freeze({init,updateBattle,notifyPreset,setPresetAttention,refresh,destroy});
})();
